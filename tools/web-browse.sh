...
cdp_eval() {
  local expr="$1"
  local params
  params=$(jq -n -c --arg expr "$expr" '{expression: $expr, returnByValue: true}')
...
main() {
  ...
  if [[ -z $browser_bin ]]; then
    echo "{\"success\":false,\"error\":\"Could not locate any valid Chrome/Chromium installation\"}"
    exit 1
  fi
  ...
  if [[ $ready == "false" ]]; then
    echo "{\"success\":false,\"error\":\"Failed to initialize remote debugger connection on port $DEBUG_PORT\"}"
    exit 1
  fi
  WS_URL=$(curl -s --noproxy "*" "http://localhost:${DEBUG_PORT}/json" | jq -rc '.[] | select(.type == "page") | .webSocketDebuggerUrl' | head -n 1)
  if [[ -z $WS_URL ]]; then
    echo "{\"success\":false,\"error\":\"No active page targets returned by remote debugger\"}"
    exit 1
  fi
  ...
  local console_logs ; console_logs=$(jq -c '.result.result.value // []' <<<"$raw_logs" 2>/dev/null)
